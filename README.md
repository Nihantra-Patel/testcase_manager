...
(Vue 3), served at `/testcase-manager`.
...
└── testcase_manager/
    ...
    ├── www/testcase-manager.html # built SPA entry page (generated)
...
`website_route_rules` in `hooks.py` maps `/testcase-manager/<path:app_path>` to
the SPA page (`www/testcase-manager.html`), so the Vue Router's history-mode
...
Then open **`https://your-site.localhost/testcase-manager`** in a browser
(you must be logged in).

> The built frontend (`public/frontend/` and `www/testcase-manager.html`) is
...
# the entry page to ../testcase_manager/www/testcase-manager.html)
yarn build
```
...
- **CI** — pre-commit checks, builds the Vue frontend, runs the server-side test
  ...
  Cypress specs headlessly via `bench run-ui-tests`. The UI job auto-detects
  whether the app ships any specs and is a green no-op when it doesn't; Cypress
  installs itself on first use (cached between runs), so no manual Cypress setup
  is needed on any OS. Unlike the in-app runner (which keeps video off — see
  above — since it has no place to show one), CI records video and, **on
  failure**, uploads the surviving recordings as a downloadable artifact —
  mirroring Frappe's own `ui-tests.yml`.
